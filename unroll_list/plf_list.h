...
#define PLF_TOOLS
	...
	private:
		...
		struct node_base
		{
			...
			node_base(const node_pointer_type &n, const node_pointer_type &p) PLF_NOEXCEPT : next(n),
																							 previous(p)
			{
			}

#ifdef PLF_MOVE_SEMANTICS_SUPPORT
			node_base(node_pointer_type &&n, node_pointer_type &&p) PLF_NOEXCEPT : next(std::move(n)),
																				   previous(std::move(p))
			{
			}
#endif
		...
		struct node : public node_base
		{
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
			node(node_pointer_type &&next, node_pointer_type &&previous, element_type &&source) PLF_NOEXCEPT : node_base(std::move(next), std::move(previous)),
			...
			node(node_pointer_type const next, node_pointer_type const previous, arguments &&...parameters) : node_base(next, previous),
		...
		struct group : public node_allocator_type // Node memory block + metadata
		{
			...
			group(const group_size_type group_size, node_pointer_type const previous = NULL) : nodes(PLF_ALLOCATE(node_allocator_type, *this, group_size, previous)),
																							   free_list_head(NULL),
			...
			group(const group_size_type group_size, node_pointer_type const previous = NULL) PLF_NOEXCEPT : nodes(NULL),
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
			group(group &&source) PLF_NOEXCEPT : node_allocator_type(source),
		...
		public:
			...
			struct ebco_pair2 : node_pointer_allocator_type // empty-base-class optimisation
			{
				...
				ebco_pair2(const size_type number_of_elements, const allocator_type &alloc) PLF_NOEXCEPT : node_pointer_allocator_type(alloc),
			...
			struct ebco_pair : group_allocator_type
			{
				...
				ebco_pair(const size_type number_of_groups, const allocator_type &alloc) PLF_NOEXCEPT : group_allocator_type(alloc),
																										capacity(number_of_groups) {};
			} group_allocator_pair;

			group_vector(const allocator_type &alloc) PLF_NOEXCEPT : allocator_type(alloc),
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
			group_vector(group_vector &&source) PLF_NOEXCEPT : allocator_type(source),
...
#define PLF_TO_ADDRESS(pointer) &*(pointer)
			...
			group_pointer_type get_nearest_freelist_group(const node_pointer_type location_node) PLF_NOEXCEPT // In working implementation this cannot throw
			{
				...
				if (location_node >= last_searched_group->nodes && location_node < last_searched_group->beyond_end) // ie. location is within last_search_group
				{
					if (last_searched_group->free_list_head != NULL)
						return last_searched_group; // if last_searched_group has previously-erased nodes
						...
						if (right_not_beyond_back)
						{
							if (location_node < right->beyond_end && location_node >= right->nodes) // location_node's group is found
							{
								...
								if (right->free_list_head != NULL)
									return right; // group has erased nodes, reuse them:
								...
								if (closest_freelist_right != NULL)
								{
									...
									if (left_distance <= 2)
										return closest_freelist_right; // ie. this group is close enough to location_node's group
	...
	private:
		...
		struct ebco_pair3 : node_allocator_type
		{
			...
			ebco_pair3(const size_type num_nodes, const allocator_type &alloc) PLF_NOEXCEPT : node_allocator_type(alloc),
	...
	public:
		...
		PLF_CONSTFUNC list() PLF_NOEXCEPT_ALLOCATOR : groups(*this),
	...
	public:
		...
		list(list &&source) PLF_NOEXCEPT : allocator_type(std::move(static_cast<allocator_type &>(source))),
...
#ifdef PLF_INITIALIZER_LIST_SUPPORT
		list(const std::initializer_list<element_type> &element_list, const allocator_type &alloc = allocator_type()) : allocator_type(alloc),
																														groups(alloc),
																														end_node(static_cast<node_pointer_type>(&end_node), static_cast<node_pointer_type>(&end_node)),
																														last_endpoint(NULL),
																														end_iterator(static_cast<node_pointer_type>(&end_node)),
		...
		list(plf::ranges::from_range_t, range_type &&rg, const allocator_type &alloc = allocator_type()) : allocator_type(alloc),
																										   end_node(static_cast<node_pointer_type>(&end_node), static_cast<node_pointer_type>(&end_node)),
																										   last_endpoint(NULL),
																										   end_iterator(static_cast<node_pointer_type>(&end_node)),
																										   begin_iterator(static_cast<node_pointer_type>(&end_node)),
	...
	private:
		...
		void insert_initialize()
		{
			...
			{
				groups.initialize(list_min_block_capacity()); // In case of prior reserve/clear call as opposed to being uninitialized
			}
			...
			if (total_size != 0)
			{
				for (group_pointer_type current_group = groups.block_pointer; current_group != groups.last_endpoint_group; ++current_group)
				{
					...
					if (end - current_group->nodes != current_group->number_of_elements) // If there are erased nodes present in the group
					{
						...
						{
							if (current_node->next != NULL && predicate(current_node->element))
								return iterator(current_node); // is not free list node and matches element
						}
					}
					else // No erased nodes in group
					{
